-- 002_add_entry_columns.sql
-- Adds HOTP to entry_type CHECK constraint and adds metadata columns
-- for efficient list queries without decryption.
--
-- SQLite cannot ALTER CHECK constraints, so we recreate the entries table.

-- Step 1: Create new table with updated CHECK constraint and new columns.
CREATE TABLE entries_new (
    id             TEXT PRIMARY KEY NOT NULL,
    entry_type     TEXT NOT NULL CHECK(entry_type IN ('totp', 'hotp', 'seed_phrase', 'recovery_code', 'secure_note')),
    name           TEXT NOT NULL,
    issuer         TEXT,
...
INSERT INTO entries_new (id, entry_type, name, issuer, folder_id, encrypted_data, created_at, updated_at)
SELECT id, entry_type, name, issuer, folder_id, encrypted_data, created_at, updated_at
FROM entries;
